...
export default function App() {
  const [lang, setLang] = useState(() => localStorage.getItem('bizlens_lang') || 'en');
  const [userEmail, setUserEmail] = useState(() => localStorage.getItem('bizlens_user_email') || '');
  ...
  useEffect(() => {
    localStorage.setItem('bizlens_lang', lang);
  }, [lang]);
      ...
      return (
        <Register 
          onRegisterSuccess={(email) => {
            setUserEmail(email);
            localStorage.setItem('bizlens_user_email', email);
          }}
    ...
    <div className="min-h-screen bg-[#080B11] text-slate-200 flex flex-col bg-grid-pattern relative overflow-x-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 pl-3 border-l border-slate-800">
              ...
              <button
                onClick={() => {
                  setUserEmail('');
                  localStorage.removeItem('bizlens_user_email');
                  setScreen('home');
                }}
      ...
      <footer className="bg-surface-container-low/40 border-t border-outline-variant/60 py-6 text-center text-xs text-slate-500">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <p>© {new Date().getFullYear()} BizLens AI. All rights reserved.</p>
