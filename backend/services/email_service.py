...
def get_or_create_ethereal_account():
    ...
    try:
        res = requests.post(
            ...
            json={'requestor': 'BizLens AI', 'version': '1.0.0'},
            timeout=10
        )
...
def send_alert_email(recipient_email: str, alerts: list, db_name: str) -> dict:
    ...
    <body style="font-family: Arial, sans-serif; color: #334155; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background-color: #ea580c; color: white; padding: 20px; border-radius: 8px 8px 0 0; text-align: center;">
            ...
            <p style="margin: 5px 0 0 0; font-size: 13px; opacity: 0.9;">BizLens AI detected critical risks in your files</p>
        </div>
        <div style="border: 1px solid #e2e8f0; border-top: none; padding: 25px; border-radius: 0 0 8px 8px;">
            ...
            <p style="margin-top: 25px;">Please log in to your BizLens dashboard to review the full details and run follow-up investigations.</p>
        </div>
        <div style="text-align: center; font-size: 11px; color: #94a3b8; margin-top: 20px;">
            This email was sent automatically by BizLens AI based on your analysis alert preferences.
        </div>
    </body>
    </html>
    """
    ...
    msg["Subject"] = f"⚠️ BizLens Alert — Action Required for {db_name}"
    msg["From"] = sender
...
def send_full_report_email(recipient_email: str, report_data: dict, db_name: str) -> dict:
    ...
    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"BizLens AI — Business Insight Report for {db_name}"
    msg["From"] = sender
    msg["To"] = recipient_email
